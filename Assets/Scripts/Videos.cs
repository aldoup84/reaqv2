...
public class Videos : MonoBehaviour
{
    ...
    public IEnumerator Cargar()
    {
        ...
        GameObject videoPlayer = GameObject.Find("Video");

        videoPlayer.AddComponent<VideoPlayer>();

#if UNITY_EDITOR
        string filePath = Path.Combine(Application.streamingAssetsPath + "/video/Video.mp4");
#elif UNITY_IOS
                        string filePath = Path.Combine (Application.streamingAssetsPath + "/Raw", urlVideo); 
#elif UNITY_ANDROID
            string filePath = Path.Combine ("jar:file://" + Application.dataPath + "!assets/video/Video.mp4");
#endif
        /*
    ...
    {
        Destroy(GameObject.Find("Video").GetComponent<VideoPlayer>());
    }
}
