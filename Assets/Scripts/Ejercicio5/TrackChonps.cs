...
using Vuforia;
public class TrackChonps : MonoBehaviour{
    ...
    protected virtual void HandleTargetStatusChanged(Status previousStatus, Status newStatus)
    {
        ...
        {
            txtElem.text = mObserverBehaviour.TargetName;
            if (shouldBeRendererNow)
            {
                OnTrackingFound();
                if (mObserverBehaviour.TargetName.Equals(txtElem.text))
                {
                    aciertos++;
                    GameObject.Find("txtTiempo2").GetComponent<RectTransform>().localScale = Vector3.zero;
                    GameObject.Find("txtAciertos").GetComponent<TextMeshProUGUI>().text = aciertos.ToString();
                    if (PlayerPrefs.GetInt("actAudio") == 1)
                    {
                        AudioSource audio = GameObject.Find("Panel3").GetComponent<AudioSource>();
                        audio.Play();
                    }
                    StartCoroutine("VerPanel", "Panel3");
                }
                else
                {
                    errores++;
                    GameObject.Find("txtErrores").GetComponent<TextMeshProUGUI>().text = errores.ToString();
                    if (PlayerPrefs.GetInt("actAudio") == 1)
                    {
                        //  Debug.Log("Reporduciendo Audio");
                        AudioSource audio = GameObject.Find("Panel4").GetComponent<AudioSource>();
                        audio.Play();
                    }
                    StartCoroutine("VerPanel", "Panel4");
                }
            }
            else
            {
    ...
    IEnumerator VerPanel(string panel)
    {
