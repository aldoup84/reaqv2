...
public class TrackElemento : MonoBehaviour {
    ...
    Text txtTarget;
    TextMeshProUGUI txtElem, txtAyuda, txtNumElem, txtPesoA, txtSimbolo, txtConcepto, txtConsultas;
    ...
    protected virtual void Start()
    {
        ...
        txtAyuda = GameObject.Find("txtAyuda").GetComponent<TextMeshProUGUI>();
        txtNumElem = GameObject.Find("txtNumElem").GetComponent<TextMeshProUGUI>();
        txtPesoA = GameObject.Find("txtPesoA").GetComponent<TextMeshProUGUI>();
        txtSimbolo = GameObject.Find("txtSimbolo").GetComponent<TextMeshProUGUI>();
        txtConcepto = GameObject.Find("txtConcepto").GetComponent<TextMeshProUGUI>();
        txtConsultas = GameObject.Find("txtConsultas").GetComponent<TextMeshProUGUI>();
        txtElem.text = "Fase 1: Elementos";
    }
    ...
    protected virtual void HandleTargetStatusChanged(Status previousStatus, Status newStatus){
        ...
        if (shouldBeRendererBefore != shouldBeRendererNow) {
            if (shouldBeRendererNow) {
                int nc = Convert.ToInt32(txtConsultas.text);
                nc++;
                if (nc > 5) {
                    if (GameObject.Find("btnEtapa2").GetComponent<RectTransform>().localScale.Equals(Vector3.zero)) {
                        GameObject.Find("btnEtapa2").GetComponent<RectTransform>().localScale = Vector3.one;
                        GameObject.Find("Panel3").GetComponent<RectTransform>().localScale = Vector3.one; //new Vector3(0.25f, 0.25f, 0.25f);
                        string inst = "Instrucciones. Coloca sobre la camara el marcador Info para visualizar la estructura molecular del compuesto quimico";
                        GameObject.Find("txtInstrucciones").GetComponent<TextMeshProUGUI>().text = inst;
                        txtElem.text = "Fase 2: Compuestos";
                        txtConcepto.text = string.Empty;
                        txtSimbolo.text = string.Empty;
                        txtPesoA.text = string.Empty;
                        txtNumElem.text = string.Empty;
                        txtAyuda.text = string.Empty;
                        GameObject.Find("Panel2").GetComponent<RectTransform>().localScale = Vector3.zero; //new Vector3(0.25f, 0.25f, 0.25f);
                        Destroy(this);
                    }
                }
                else {
                    txtConsultas.text = nc.ToString();
                    Debug.Log("Trackable " + mObserverBehaviour.TargetName + " found");
                    OnTrackingFound();
                    try {
                        Elemento elem = DatosEj1.Instance.getAyuda(mObserverBehaviour.TargetName);
                        GameObject.Find("Panel2").GetComponent<RectTransform>().localScale = Vector3.one;
                        txtConcepto.text = "Elemento: " + elem.descripcion + "  (" + elem.simbolo + " )";
                        txtPesoA.text = "Masa Atomica:  " + elem.pesoAtomico.ToString();
                        txtNumElem.text = "Numero Atómico:  " + elem.numElemento.ToString();
                        txtAyuda.text = elem.ayuda;
                        GameObject.Find("avance").GetComponent<Button>().GetComponent<UnityEngine.UI.Image>().fillAmount += 0.20f;
    ...
    IEnumerator VerPanel() {
        ...
        if (nc >= 5) {
            if (GameObject.Find("btnEtapa2").GetComponent<RectTransform>().localScale.Equals(Vector3.zero)) {
                GameObject.Find("btnEtapa2").GetComponent<RectTransform>().localScale = Vector3.one;
                GameObject.Find("Panel3").GetComponent<RectTransform>().localScale = Vector3.one; //new Vector3(0.25f, 0.25f, 0.25f);
                string inst = "Instrucciones. Coloca sobre la camara el marcador Informacion para visualizar la estructura moleculardel compuesto quimico";
                GameObject.Find("txtInstrucciones").GetComponent<TextMeshProUGUI>().text = inst;
                txtElem.text = "Fase 2: Compuestos";
            }
        }
    ...
    public void Guardar(int aciertos) {
        ...
        FBManager.getInstance().SaveExercise(new Ejercicio2("Reaq v2.2", "IDElementos", idUsuario, "Facil", aciertos, 0, 0, (float)(tiempo), aciertos * 100, 1, fecha, hora));
    }
    ...
    protected bool ShouldBeRendered(Status status) {
        ...
        if (StatusFilter == TrackingStatusFilter.Tracked_ExtendedTracked_Limited && (status == Status.EXTENDED_TRACKED || status == Status.LIMITED)){
             return true; // in this mode, render the augmentation even if the target's tracking status is LIMITED. this is mainly recommended for Anchors.
        }
        return false;
    }
    ...
    class PoseSmoother {
        const float e = 0.001f, MIN_ANGLE = 2f;
        ...
        /// Smooth pose transition if the pose changed and the target is still being reported as "extended tracked" or it has just returned to "tracked" from previously being "extended tracked"
